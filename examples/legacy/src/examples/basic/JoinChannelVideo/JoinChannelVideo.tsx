...
const banubaClientToken =
  'YOUR_BANUBA_CLIENT_TOKEN';
const banubaProvider = 'Banuba';
const banubaExtension = 'BanubaFilter';
const banubaPlugin = 'banuba-plugin';
  ...
  implements IRtcEngineEventHandler
{
  ...
  protected async initRtcEngine() {
    ...
    this.initializeBanuba();
    this.loadBanubaEffect('Glasses');
  ...
  switchCamera = () => {
    ...
    this.setState((preState) => {
      const switchCamera = !preState.switchCamera;
      this.setBanubaProperty('set_require_mirroring', switchCamera ? '0' : '1');
      return { switchCamera };
    });
  };
  ...
  protected releaseRtcEngine() {
    this.setBanubaProperty('release', 'null');
    this.engine?.enableExtension(banubaProvider, banubaExtension, false);
  ...
  private enableBanubaExtension() {
    if (Platform.OS === 'android') {
      this.engine?.loadExtensionProvider('banuba');
      this.engine?.loadExtensionProvider(banubaPlugin);
    }

    const result = this.engine?.enableExtension(
      banubaProvider,
      banubaExtension,
      true
    );
    if (result !== 0) {
      throw new Error(`Failed to enable Banuba extension: ${result}`);
    }
  }

  private initializeBanuba() {
    this.setBanubaProperty('set_banuba_license_token', banubaClientToken);
    this.setBanubaProperty('set_require_mirroring', '1');
  }

  private loadBanubaEffect(name: string) {
    this.setBanubaProperty('load_effect', `effects/${name}`);
  }

  private setBanubaProperty(key: string, value: string) {
    const result = this.engine?.setExtensionProperty(
      banubaProvider,
      banubaExtension,
      key,
      value
    ...
    if (result !== 0) {
      this.error(`Failed to set Banuba property "${key}": ${result}`);
    }
  }
